...
<body>
    ...
    <main class="container mt-5">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            include 'db.php'; // Assurez-vous que ce fichier gère la connexion à la base de données de manière sécurisée

            $email = mysqli_real_escape_string($conn, $_POST['email']);
            $password = $_POST['password']; // Ce mot de passe doit être haché et vérifié
            ...
            if ($result->num_rows > 0) {
                ...
                if (password_verify($password, $user['hashed_password'])) {
                    // Gestion de session après une connexion réussie
                } else {
